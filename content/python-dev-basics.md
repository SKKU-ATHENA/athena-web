...
<div class="flow" style="flex-wrap: wrap; gap: 0.5rem;">
  <div class="flow-badge flow-badge-primary">API 호출</div>
  <div class="flow-arrow">→</div>
  <div class="flow-badge flow-badge-primary">파일 I/O</div>
  <div class="flow-arrow">→</div>
  <div class="flow-badge flow-badge-primary">JSON 처리</div>
  <div class="flow-arrow">→</div>
  <div class="flow-badge flow-badge-primary">비동기 처리</div>
</div>

<div class="compare-grid" style="grid-template-columns: repeat(2, 1fr);">
  <div class="info-card">
    <div class="info-card-title">API 호출</div>
    Ollama/OpenAI/Anthropic에 텍스트를 보내고 응답을 받는다
  </div>
  <div class="info-card">
    <div class="info-card-title">파일 I/O</div>
    문서를 읽고, 처리 결과를 저장한다
  </div>
  <div class="info-card">
    <div class="info-card-title">JSON 처리</div>
    API 응답과 설정 파일은 대부분 JSON 형식이다
  </div>
  <div class="info-card">
    <div class="info-card-title">비동기 처리</div>
    여러 API 호출을 동시에 보내 처리 속도를 높인다
  </div>
</div>
...
**API(Application Programming Interface)**란 프로그램끼리 데이터를 주고받기 위한 약속된 창구다. 식당의 주문 카운터에 비유할 수 있다 — 메뉴(요청 형식)에 맞게 주문하면, 주방(서버)이 요리(응답)를 내놓는다. **HTTP 요청**은 이 창구에 주문을 넣는 행위로, 웹에서 데이터를 주고받는 표준 방식이다. 브라우저가 웹페이지를 불러올 때도 HTTP 요청을 사용한다.

LLM API도 HTTP 요청으로 통신한다. Python에서 HTTP 요청을 보내는 대표적인 라이브러리는 `requests`와 `httpx` 두 가지다.
...
**캐싱(caching)**이란 한 번 받은 결과를 저장해두고, 같은 요청이 오면 저장된 결과를 재사용하는 기법이다. 마트에서 자주 사는 물건을 집에 비축해두는 것과 같다 — 매번 사러 갈 필요가 없어진다. LLM API는 느리고 비용이 들기 때문에, 같은 질문의 응답을 캐싱하면 재호출을 피할 수 있다.
...
<div class="compare-grid">
  <div class="compare-item compare-before">
    <div class="compare-label">동기 (Synchronous)</div>
    <div class="flow" style="flex-wrap: wrap;">
      <div class="flow-badge flow-badge-red">호출1 ████</div>
      <div class="flow-arrow">→</div>
      <div class="flow-badge flow-badge-red">호출2 ████</div>
      <div class="flow-arrow">→</div>
      <div class="flow-badge flow-badge-red">호출3 ████</div>
    </div>
    <br><strong>총 9초</strong> — 순차 실행, 앞 작업이 끝나야 다음 시작
  </div>
  <div class="compare-item compare-after">
    <div class="compare-label">비동기 (Asynchronous)</div>
    <div style="display: flex; flex-direction: column; gap: 0.3rem;">
      <div class="flow-badge flow-badge-green">호출1 ████</div>
      <div class="flow-badge flow-badge-green">호출2 ████</div>
      <div class="flow-badge flow-badge-green">호출3 ████</div>
    </div>
    <br><strong>총 3초</strong> — 동시 실행, 대기 시간에 다른 작업 진행
  </div>
</div>
...
#### TypedDict — 딕셔너리에 타입 안전성 더하기

일반 딕셔너리(`dict`)는 어떤 키든 넣을 수 있어 자유롭지만, 오타가 있어도 실행 전까지 알 수 없다. `TypedDict`는 "이 딕셔너리에는 이런 키가 있고, 각 키의 값은 이런 타입이다"라고 미리 선언하는 것이다. API 응답처럼 구조가 정해진 데이터에 쓰면, IDE가 키 이름을 자동완성하고 오타를 즉시 잡아주므로 팀 프로젝트에서 실수를 크게 줄일 수 있다.
...
ATHENA 프로젝트에서는 `uv`를 패키지 매니저로 사용하는 것을 고려하고 있다. pip보다 10~100배 빠르고, 사용법은 거의 같다.
...
아래 질문에 답할 수 있으면 이 자료의 핵심을 이해한 것이다. 스스로 생각해본 뒤, 답을 펼쳐 확인하자.

<details>
<summary>Q1. httpx.post()에서 json= 파라미터와 data= 파라미터의 차이는?</summary>

`json=`은 딕셔너리를 자동으로 JSON 문자열로 변환하고 `Content-Type: application/json` 헤더를 설정한다. `data=`는 문자열이나 바이트를 그대로 보낸다. LLM API 호출에는 `json=`을 쓴다.

</details>

<details>
<summary>Q2. asyncio.gather()는 무엇을 하는 함수인가?</summary>

여러 비동기 작업(코루틴)을 동시에 실행하고, 모든 작업이 완료될 때까지 기다린 뒤 결과를 리스트로 반환한다. 10개의 API 호출을 순차가 아닌 병렬로 처리할 수 있다.

</details>

<details>
<summary>Q3. json.dumps()에서 ensure_ascii=False를 빼면 어떤 일이 발생하는가?</summary>

한국어 텍스트가 `\uD55C\uAD6D\uC5B4`처럼 유니코드 이스케이프 시퀀스로 변환된다. 파일을 열었을 때 사람이 읽을 수 없게 되므로, 한국어 프로젝트에서는 반드시 `ensure_ascii=False`를 사용한다.
